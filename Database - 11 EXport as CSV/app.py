    ...
    if new_table_name:
        with connect_db() as connection:
            ...
            try:
                ...
                flash(f"Table '{new_table_name}' created successfully.", 'success')
            except sqlite3.OperationalError as e:
                flash(f"Error creating table: {e}", 'danger')
    else:
        flash("Please provide a valid table name.", 'danger')

    return redirect(url_for('display_tables'))

# Add this route to handle bulk actions

@app.route('/perform_bulk_action/<table_name>', methods=['POST'])
def perform_bulk_action(table_name):
    selected_rows = request.form.getlist('selected_rows')

    if 'export_csv' in request.form:
        # Export selected rows as CSV
        return export_selected_as_csv(table_name, selected_rows)

    with connect_db() as connection:
        cursor = connection.cursor()
        delete_query = f"DELETE FROM {table_name} WHERE rowid IN ({', '.join(['?']*len(selected_rows))});"
        cursor.execute(delete_query, selected_rows)
        connection.commit()

    flash(f"Selected rows deleted from '{table_name}' table.", 'success')
    return redirect(url_for('display_tables'))

def export_selected_as_csv(table_name, selected_rows):
    with connect_db() as connection:
        cursor = connection.cursor()
        select_query = f"SELECT * FROM {table_name} WHERE rowid IN ({', '.join(['?']*len(selected_rows))});"
        cursor.execute(select_query, selected_rows)
        rows = cursor.fetchall()
    ...
    # Write CSV header
    csv_writer.writerow(fetch_column_names(table_name))
    ...
    response = Response(
        csv_output.getvalue(),
        mimetype='text/csv',
        content_type='attachment; filename=export.csv'
    )
